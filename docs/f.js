...
async function gubs() {
	...
	bal = await Promise.all([
		...
		fa.earned(window.ethereum.selectedAddress,TEARNED[1]),
...
async function extendRewards() {
	notice(`
		...
		Enter the amount of ${TEARNSYM[1]} you want to Donate as rewards to this Farmland. It will also extend the dry date by 8 days!<br><br>
...
async function notifyRewards() {
	...
	_T = new ethers.Contract(TEARNED[1],LPABI,signer);
	...
	txh = await _F.notifyRewardAmount( TEARNED[1], BigInt(Math.floor(_amt*1e18)) );
